...
> 以下是用户明确提出的需求方向，按优先级排序。每个方向包含具体要求和验收标准。

### 🔴 高优先级

#### 1. Service Worker 版本升级
- **现状**：`public/sw.js` 缓存版本仍为 `mnemo-v1`，用户可能看到旧缓存
- **要改**：升级到 `mnemo-v3` 或更高
- **文件**：`frontend/public/sw.js`

#### 2. 部署今天所有改动
- practice 模块重建 + 全程 AI 辅导 + tasks 拆分 + 错因细化 + 难度评估
- 部署流程：`scripts/deploy-backend.sh` + 前端 `out/` 目录复制到 Nginx
- 部署后提醒用户 `Ctrl+Shift+R` 强刷

#### 3. memory/page.tsx 拆分
- **现状**：2354 行，状态高度耦合，功能正常但维护困难
- **建议拆法**：提取 hooks（useMemorySession、useCardReview 等）+ 6 个 UI 子组件
- **注意**：这是最复杂的页面，拆分前务必先理解完整逻辑，建议先写测试

---

### 🟡 中优先级 — LLM 智能感落地

> 用户原话："LLM 智能感缺失"。当前系统虽然用了 LLM，但很多地方只是简单调用，没有让用户感受到 AI 的"智能"。以下是需要加强的方向：

#### 4. 学习记录 Tab 升级 + LLM 每日摘要/周报
- **现状**：dashboard 有基础统计，但没有 AI 生成的学习洞察
- **要做的**：
  - 每日学习结束时（或次日打开时），LLM 自动生成前一天的学习摘要
  - 内容包括：学了什么、正确率变化、薄弱知识点、建议下一步
  - 每周生成周报：本周学习时长、掌握进度、与上周对比、下周建议
  - 用自然语言，像私教一样跟用户对话，不要干巴巴的数据罗列
- **技术方案**：
  - 后端新增 `POST /ai/daily-summary` 和 `POST /ai/weekly-report`
  - 输入：当天的记忆卡片练习记录 + 练习助手记录 + 任务完成情况
  - 输出：LLM 生成的自然语言摘要
  - 前端在 dashboard 或 memory 页面展示
- **验收标准**：用户打开 app 能看到"昨天你学了 XX，语法正确率从 60% 提升到 75%，建议今天重点复习虚拟语气"

#### 5. 任务智能规划
- **现状**：任务管理是纯手动的，用户自己创建/勾选任务
- **要做的**：
  - 用户设置备考目标（已有 goal-picker）后，LLM 自动生成每日/每周学习计划
  - 根据用户的薄弱点（错题统计、CASR 置信度低的卡片）智能推荐今天的任务
  - 计划要具体："今天背 20 个翻译词条（重点：法律术语）+ 做 5 道语法选择题 + 复习 10 张错题卡片"
  - 不是笼统的"今天要好好学习"
- **技术方案**：
  - 后端新增 `POST /ai/plan` 接口
  - 输入：备考目标 + 历史学习数据 + 薄弱点分析
  - 输出：结构化的每日任务列表（可直接导入 tasks 表）
  - 前端在 dashboard 或 tasks 页面加"AI 帮我规划今天"按钮
- **验收标准**：用户点一下按钮，AI 就根据他的实际情况生成一份当天的具体学习清单

#### 6. PDF 题册导入
- **现状**：练习模块只有"对话式生成"一个输入通道
- **用户需求**："出门在外不带书/卷子，需要手机端聚合题目"
- **要做的**：
  - 用户上传 PDF 题册（可能是扫描件）
  - PyMuPDF 按页切图
  - VLM（qwen-vl）识别题目边界，结构化输出：题目、选项、答案、解析、题型、考点
  - 用户预览确认（可编辑/删除/调整分类）
  - 入库后和生成式题目统一管理
- **技术方案**：
  - 后端新增 `POST /practice/import-pdf` 接口
  - 用 PyMuPDF 提取文本，提取失败的页面走 VLM
  - LLM 结构化解析（类似 schedule_import 的 OCR 模式）
  - 前端加上传入口 + 预览编辑界面
- **注意**：这是练习模块最后一块拼图，技术复杂度最高
- **验收标准**：用户上传一本语法书 PDF，系统能自动识别出题目并入库
...
#### 7. 桌面端 Tauri 集成
- 已有 `desktop/` 目录骨架
- 需要配置 Tauri 打包 + 原生功能（通知、托盘等）

#### 8. 安卓端 Capacitor 打包
- 已有 `mobile/capacitor.config.json`
- 需要配置 Android Studio 打包 + 推送通知

#### 9. 其他页面拆分
- `schedule/page.tsx`（966 行）— 逻辑全在一个函数内，拆分收益低
- `courses/page.tsx`（701 行）— NotesTab/ChatTab 已是自包含组件

#### 10. 数据导出
- 卡片/任务/练习记录导出为 Excel/PDF

#### 11. 全局搜索
- 已删除（占地方且与任务搜索重复），可能需要重新设计

---

### 用户反馈的 6 大打磨方向（原始需求）

> 这是用户在一次反馈中提出的 6 个方向，部分已完成，部分待做：

| # | 方向 | 状态 | 说明 |
|---|---|---|---|
| 1 | 记忆模式太少 | ✅ 已完成 | 5 种训练模式 + 错因感知智能推荐 |
| 2 | 难度分布形同虚设 | ✅ 已完成 | 导入时启发式评估 + 复习时动量模型动态调整 |
| 3 | 周报日历粗糙 | 🟡 部分 | 基础统计有，LLM 周报未做（见上方第 4 项） |
| 4 | 错因太笼统 | ✅ 已完成 | 9 种错因分类 + AI 自动诊断 + 颜色编码 |
| 5 | LLM 智能感缺失 | 🟡 部分 | 练习模块 AI 辅导已完成，学习摘要/智能规划未做（见上方第 4、5 项） |
| 6 | 桌面端待跟进 | ⬜ 未开始 | Tauri 集成（见上方第 7 项） |
